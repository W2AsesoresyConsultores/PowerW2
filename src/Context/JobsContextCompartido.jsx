...
const JobsContextCompartido = createContext();

const removeAccents = (str) => {
  return str ? str.normalize("NFD").replace(/[\u0300-\u036f]/g, "") : "";
};

export const JobsProviderCompartido = ({ children }) => {
  const { user } = UserAuth();
  ...
  const fetchJobs = async () => {
    ...
    const idEmpresa = profileData.id_empresa; // Cambiar a id_empresa
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'Oferta' }, (payload) => {
        ...
        if (newJob.id_empresa === idEmpresa) { // Cambiar a id_empresa
  ...
  return (
    <JobsContextCompartido.Provider value={{
      jobs,
      setJobs,
      searchJobs,
      userSearchResults,
      ...
      {children}
    </JobsContextCompartido.Provider>
  );
};

export default JobsContextCompartido;